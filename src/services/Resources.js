...
export default class Resources {
  async find(params = null) {
    ...
    let result = await ApiManager.get(
      this.routes.find || this.routes.resource,
      params,
    );
    if (this.afterRemote) {
  ...
  async findById(id) {
    ...
    let result = await ApiManager.get(
      `${this.routes.findById || this.routes.resource}/${id}`,
    );
    if (this.afterRemote) {
  ...
  async create(payload, isUrlEncoded = false) {
    ...
    let result = await ApiManager.post(
      `${this.routes.create || this.routes.resource}`,
      payload,
      isUrlEncoded,
    );
  ...
  async updateAttributes(payload, id, config) {
    ...
    let result = await ApiManager.patch(
      `${this.routes.updateAttributes || this.routes.resource}/${id}`,
      payload,
      config,
    );
